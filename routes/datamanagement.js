/*
This part of the code receives the request from the UI tree. The id parameter indicates the node 
that is being expanded: 
# means root node, so list hubs. After that it contains the href of the resource, so when expanding 
one hub the endpoint should return the projects for the hub. This code calls different get functions.
*/
...
/* 
The last get function returns the Versions for each item (file), where the .relationships.derivatives.data.id 
property contains the URN for the Viewer. It's important to test if this attribute is available as some 
items may not have viewables (e.g. a ZIP or DOCx file) or may not have being translated yet.

Note the reuse of auth helpers from routes/common/oauth.js.
*/
